...
async def main():
    ...
    await send_dialogue(
        "Hiccup",
        "Okay, so this is my workshop, it's a bit beaten up wit Grump around but it's nicely located next to Gobber's forge! Anywho, if you need anything to make, you'll always a hand or two.",
    )
    await send_story("*A Night Fury growls*")
    await send_dialogue(
        "Hiccup",
        f"Yes, Toothless, and a paw.... Oh, meet Toothless! Toothless, {await world.player.name}. {await world.player.name}, Toothless. I hope you're not too scared of him. He's really kind actually -- all dragons are.",
    )
    await send_dialogue(
        await world.player.name,
        "Hiccup, I think I should get going. The village won't introduce myself, right?",
    )
    await send_dialogue(
        "Hiccup",
        "Right! Sure, there's the exit, you should be right at the arena after that.",
    )
