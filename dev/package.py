...
def migrate_quest(id: str, code: str, version: tuple[int, int, int], target_dir: str):
    ...
    if os.path.exists(file_name):
        if os.path.isfile(file_name):
            ...
            if old_code != code:
                raise QuestVersionExistsException(
                    f"Cannot write quest {id}, version {'.'.join(list(map(str, version)))}: Quest exists"
                )
        else:
            raise FileExistsError(
                f"Cannot write quest file to {file_name}: Is a directory"
            )
    else:
        ...
        if os.path.isfile("content/quest/" + quest) and quest.endswith(".py"):
            ...
            with open(f"content/quest/{quest}", "r") as f:
                ...
                try:
                    ...
                    if debug:
                        print(
                            f"Note: overwriting quest {id}, version {'.'.join(list(map(str, version)))}:"
                        )
                    else:
                        raise
